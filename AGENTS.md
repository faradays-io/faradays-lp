...
# Páginas legais (`/politicas`, `/termos`, `/privacidade`, `/subprocessadores`, `/suporte`, `/cookies`, `/licencas`)

- **Fonte única**: `src/components/landing/legal-data.ts` — `LEGAL_PAGES` (slug, label curto, título e descrição do índice), `LEGAL_INDEX` (`/politicas`) e `FOOTER_LEGAL_LINKS` (subconjunto dos rodapés de produto). Nova política = nova entrada aqui + rota em `src/app/<slug>/page.tsx`.
- **Casca**: `LegalPage` (`legal-page.tsx`) recebe `content: Localized<LegalContent>`. Cada seção tem `heading` e `body: LegalBlock[]`, onde string = parágrafo e objetos cobrem `list`, `table` (`head` + `rows`) e `sub` (subseção). Texto inline aceita `[rótulo](href)` e `**negrito**` — sem JSX no conteúdo. `numbered: true` liga numeração (1., 3.1…) e o sumário com âncoras; `originalAt` + `updatedAt` são as datas do topo.
- **Verdade antes de promessa**: os textos descrevem o que a plataforma faz de fato. Antes de alterar uma afirmação factual (fornecedor, retenção, cookie, segurança), confirmar no código do produto e registrar em `docs/politicas-pendencias.md`. Trocar subprocessador exige atualizar `/subprocessadores` e avisar clientes ativos com 30 dias — é o que o Aviso promete.
- **EN é tradução de trabalho do PT**; em conflito, vale o PT. Nenhum dos textos passou por revisão jurídica.
- **Estilo dos textos legais**: registro seco do site (frase curta, concreta, sem adjetivo; "você" para o visitante, "Cliente" para a empresa contratante), com o rigor formal do modelo de referência (ajuda.zdg.com.br): termos definidos sempre com inicial maiúscula e iguais nos dois documentos — Site, Plataforma, Cliente, Usuário, Representante, Titular, Plano, Dados do Cliente, Subprocessador, Encarregado; bullets no formato `**Termo.** frase`; artigos da LGPD citados (`art. 7º, V`); numerais sem forma por extenso ("5 anos", não "5 (cinco) anos"); frases acima de ~35 palavras são quebradas. Fornecedor citado leva link para a política dele.
